...
class AFMAnalyzer:
    ...
    def __init__(self, file_path):
        ...
        self.px_size_um = None       # μm per pixel
    ...
    # -------------------------------------------------------------------------
    # Plotting — existing channels / roughness / PSD
    # -------------------------------------------------------------------------
    ...
    def plot_particle_labels(self):
        ...
        im = axes[0].imshow(self.height_leveled, cmap='afmhot', origin='lower', extent=extent)
    ...
    def plot_population_statistics(self):
        """Plot 2x2 panel of particle size distribution, height distribution,
        height-vs-diameter scatter, and spatial particle map."""
        if self.particle_df is None:
            self.extract_particle_features()
    ...
    def save_particle_map(self, filename="particle_map.png", dpi=150):
        ...
        fig.savefig(out_path, dpi=dpi, bbox_inches="tight")
        plt.close(fig)
        print(f"Saved particle map to: {out_path}")
